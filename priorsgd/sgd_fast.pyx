...
cimport cython
from libc.math cimport exp, log, sqrt, pow, fabs, log10, floor
...
from weight_vector cimport WeightVector
from seq_dataset cimport SequentialDataset
...
def plain_sgd(np.ndarray[double, ndim=1, mode='c'] weights,
              ...
              double l1_ratio,
              np.ndarray[double, ndim=1, mode='c'] per_feature_alpha, 
              np.ndarray[double, ndim=1, mode='c'] per_feature_beta, 
              np.ndarray[double, ndim=1, mode='c'] modal_vector,
    ...
    per_feature_alpha : ndarray[double, ndim=1]
        The per feature L2 regularization strength, this strength specify 
        the actual constant in the target function being optimized instead of 
        the ones in the gradient.
        The length should be identical to the number of features.
        The scale of the strength is in per example basis.
    per_feature_beta : ndarray[double, ndim=1]
        The per feature L1 regularization strength.
        The length should be identical to the number of features.
        The scale of the strength is in per example basis.
    modal_vector : ndarray[double, ndim=1]
        The vector that the weight vector would be regulating toward. 
        The length should be identical to the number of features.
...
def _plain_sgd(np.ndarray[double, ndim=1, mode='c'] weights,
    ...
    cdef double * pf_alpha_ptr = &per_feature_alpha[0]
    cdef double * pf_beta_ptr = &per_feature_beta[0]
    cdef double * modal_data_ptr = &modal_vector[0]


    # Eugene: pass the regularization information to the weight vector
    cdef WeightVector w = WeightVector(weights, average_weights, 
                                       per_feature_alpha, per_feature_beta, 
                                       modal_vector, 
                                       n_samples)
    ...
    cdef double l1dist = 0.0
    cdef double l2dist = 0.0
    cdef double y = 0.0
    ...
    cdef bint convergence_test = tol > -INFINITY
    ...
    with nogil:
        for epoch in range(max_iter):
            ...
            for i in range(n_samples):
                ...
                # Eugene: apply the loss gradient from the example
                if update != 0.0:
                    w.add(x_data_ptr, x_ind_ptr, xnnz, update)
                    if fit_intercept == 1:
                        intercept += update * intercept_decay
                
                # Eugene: apply both l1 and l2 regularization
                w.apply_penalty(eta, x_ind_ptr, xnnz)
            ...
            if convergence_test or verbose:
                # Eugene: the `w.norm()` and `w.sq_norm` will not be maintained anymore
                # Eugene: We calculate the l1 and l2 distance to the modal vector here
                l1dist = 0.0
                l2dist = 0.0
                for j in range(n_features):
                    l1dist += fabs( w_ptr[j] - modal_data_ptr[j] ) * pf_beta_ptr[j]
                    l2dist += pow( ( w_ptr[j] - modal_data_ptr[j] ), 2 ) * pf_alpha_ptr[j]
                l2dist = l2dist * n_samples
                l1dist = l1dist * n_samples

                # report epoch information
                if verbose > 0:
                    with gil:
                        print("Norm: %.2f, NNZs: %d, Bias: %.6f, T: %d, "
                            "Logistic loss: %f, L1 loss: %f, L2 loss: %f, "
                            "Total loss: %f"
                            % (w.norm(), weights.nonzero()[0].shape[0],
                                intercept, count, sumloss, l1dist, l2dist,
                                sumloss+l1dist+l2dist ))
                        print("Total training time: %.2f seconds."
                            % (time() - t_start))

                # Eugene: measure the total loss instead of just the loss on the 
                # examples
                sumloss += ( l1dist + l2dist )
